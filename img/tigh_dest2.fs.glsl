...
void main() {
    ...
    vec4 col0 = texture(shampler, tc0) * 1.0;
    vec4 col1 = texture(shampler, tc1) * 2.0;
    vec4 col2 = texture(shampler, tc2) * 1.0;
    vec4 col3 = texture(shampler, tc3) * 2.0;
    vec4 col4 = texture(shampler, tc4) * 4.0;
    vec4 col5 = texture(shampler, tc5) * 1.0;
    vec4 col6 = texture(shampler, tc6) * 2.0;
    vec4 col7 = texture(shampler, tc7) * 1.0;
    vec4 col8 = texture(shampler, tc8) * 2.0;

    color = (col4 + col1 + col3 + col5 + col7) * 0.2;// + col2 + col6 + col8) / 15;
}
